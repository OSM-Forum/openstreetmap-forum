...
// Update last_seen_ip for each page visit of logged in users
if (!$pun_user['is_guest'])
    $db->query('UPDATE '.$db->prefix.'users SET last_seen_ip=\''.$db->escape(get_real_remote_address()).'\', last_seen='.time().' WHERE id='.$pun_user['id']) or error('Unable to update user data', __FILE__, __LINE__, $db->error());
